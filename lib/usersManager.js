...
exports.createRoom = function(data, callback){
    ...
    db.query(queryString, function(err){
        if(err){
            ...
            for(var i = 0; i < data.alumns.length; i++){
...
exports.findAlumnRooms = function(alumn, callback){
...
exports.findVideos = function(student, callback){
    var queryString = "SELECT room_name, teacher, rec_date, teacher_video, screen_video  FROM classroom c " +
        "JOIN asists a ON c.room_name = a.classroom JOIN videos v ON c.room_name = v.room WHERE alumn ='" +student+ "';";
    db.query(queryString, function(err, result){
        if(err){
            return callback(err);
        }else{
            ...
            for(var i = 0; i < result.rows.length; i++) {
                list[i] = {
                    room_name: result.rows[i].room_name,
                    rec_date: result.rows[i].rec_date,
                    teacher: result.rows[i].teacher,
